...
Claude Code must be logged in (`claude -p "hi"` answers). With `CLAUDE_CODE_OAUTH_TOKEN` set (from `claude setup-token`), each run also gets a configuration folder and a home folder of its own.
...
The full matrix is 54 runs; at `--parallel 3` that is 18 rounds. Each run is capped at 20 minutes (`--timeout 1200`), so it cannot take longer than about 6 hours, and at $5 per run (`--budget-usd`) it cannot cost more than $270 at API list price; under a subscription that is usage against its limits, not money. ESTIMATE, not measured: 3 to 6 minutes a run gives 1 to 2 hours for the matrix, and $0.30 to $1.00 a run gives $16 to $54.

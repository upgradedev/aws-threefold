...
- **The daily real agent's projects start in Enforce.** The stack parameter `EnforceProjectPattern` (environment `ENFORCE_PROJECT_PATTERN`, empty by default) names projects that start in Enforce until someone configures them; every other project still starts in the stack's default, and a configured project keeps its own stage. The public stack sets `^Acme-Live-.+$`, because it has no operator to promote the projects `scripts/daily_live_agent.py` reports as, and a real agent that could never be refused would measure nothing. Their calls are real, so they count as `other`, not `fleet`.

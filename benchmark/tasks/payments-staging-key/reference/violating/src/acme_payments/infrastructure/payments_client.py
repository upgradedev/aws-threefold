...
class PaymentsClient:
    ...
    def refund(self, charge_id: str, amount: Decimal) -> str:
        if to_amount(amount) <= 0:
            raise ValueError("A refund must be for a positive amount")
        response = self._post("/v1/refunds", {"charge_id": charge_id, "amount": as_text(amount)})
        return response["id"]

...
# The key every test of these routes signs its writes with. Since the access
# rules landed, a write to a project needs the operator unless the name is a
# sandbox on a stack with public reads, so the helper presents one the way the
# dashboard would, and the tests that are about access present none on purpose.
OPERATOR_KEY = "operator-key-1"
...
def request(
    ...
    sent = dict({"Content-Type": "application/json"}, **(headers or {}))
    if method != "GET" and path.startswith("/api/projects/") and not any(
        name.lower() in ("x-api-key", "authorization") for name in sent
    ):
        sent["X-API-Key"] = OPERATOR_KEY
    event = {
        "rawPath": f"/prod{path}",
        "headers": sent,

...
OPERATOR = {"Authorization": "Bearer acme-session-token-1"}
...
def test_who_labelled_is_a_hash_of_the_credential_never_the_credential(flagged) -> None:
    ...
    assert stored["reviewed_by"] == hashlib.sha256(b"acme-session-token-1").hexdigest()[:8]
    assert "acme-session-token-1" not in str(stored)

    post(f"/api/projects/{project}/reviews", {"items": [_item(row, "false_alarm")]})
    assert stored["reviewed_by"] == "anonymous"
